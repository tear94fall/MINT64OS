...
START:
	...
	; 스택을 0x000:0000~0x0000:FFFF 영역에 64KB 크기로 생성
	mov ax, 0x0000	; 스택 세그먼트의 시작 어드레스(0x0000)를 세그먼트 레지스터 값으로 변환
	mov ss, ax		; SS 세그먼트 레지스테어 설정
...
RESETDISK:								; 디스크를 리셋하는 코드의 시작
	...
	mov bx, 0x0000						; BX 레지스터에 0x0000를 설정하여 복사할
...
.MESSAGELOOP:					; 메시지를 출력하는 루프
	mov cl, byte [ si ]			; SI 레지스터가 가리키는 문자열 위치에서 한문자를
								; CL 레지스터에 복사
	...
	je .MESSAGEEND				; 복사한 문자의 값이 0이면 문자열이 종료되었음을 의미하므로
								; .MESSAGEEND로 이동하여 문자열 출력 종료

	mov byte [ es: di ], cl		; 0이 아니면 비디오 메모리 어드레스 0xB800:di 에 문자열 출력
