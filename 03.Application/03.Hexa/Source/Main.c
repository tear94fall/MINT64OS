...
int Main( char* pcArgument )
{
    ...
    while( 1 )
    {
        ...
        // 게임 루프 처리 부분
        //------------------------------------------------------------------------------------------
        // 게임이 시작되었다면 레벨에 따라 일정 시간 대기한 뒤에 블록을 아래로 이동
        if( ( g_stGameInfo.bGameStart == TRUE ) && ( ( GetTickCount() - qwLastTickCount ) > ( 300 - ( g_stGameInfo.qwLevel * 10 ) ) ) )
        {
            ...
            if( IsMovePossible( g_stGameInfo.iBlockX, g_stGameInfo.iBlockY + 1 ) == FALSE )
            {
                ...
                // 보드에 블록을 검사해 세개 이상 연속된 블록을 삭제하고 화면에 표시
...
//  가로 방향으로 일치하는 블록을 찾아서 표시
{
    ...
    // 게임판 전체를 검색해 가로 방향으로 세 개 이상인 것을 찾아서 표시
    for( j = 0 ; j < BOARDHEIGHT ; j++ )
    {
        ...
        for( i = 0 ; i < BOARDWIDTH ; i++ )
        {
            ...
            else
            {
                ...
                {
                    iMatchCount++;
                    // 연속된 블록이 세 개면 모아둔 이전 블록 세 개를 지움 블록으로 표시
...
BOOL MarkContinuousVerticalBlockOnBoard( void )
{
    ...
    bMarked = FALSE;
    // 게임판 전체를 검색해 세로 방향으로 세개 이상인 것을 찾아서 표시
    for( i = 0 ; i < BOARDWIDTH ; i++ )
    {
        iMatchCount = 0;
        bBlockKind = 0xFF;
        ...
        for( j = 0 ; j < BOARDHEIGHT ; j++ )
        {
            ...
            else
            {
                ...
                {
                    iMatchCount++;
                    // 연속된 블록이 세 개면 모아둔 이전 3블록을 지움 블록으로 표시
                    if( iMatchCount == BLOCKCOUNT )
                    {
                        for( k = 0 ; k < iMatchCount ; k++ )
                        {
...
BOOL MarkContinuousDiagonalBlockInBoard( void )
{
    ...
    //------------------------------------------------------------------------------------------
    // 게임판 전체를 검색해 위에서 아래의 대각선 반향으로 세 개 이상인 것을 찾아서 표시
    //------------------------------------------------------------------------------------------
    for( i = 0 ; i < BOARDWIDTH ; i++ )
    {
        for( j = 0 ; j < BOARDHEIGHT ; j++ )
    ...
    //------------------------------------------------------------------------------------------
    // 게임판 전체를 검색해 아래에서 위의 대각선 방향으로 세 개 이상인 것을 찾아서 표시
    //------------------------------------------------------------------------------------------
    for( i = 0 ; i < BOARDWIDTH ; i++ )
    {
        for( j = 0 ; j < BOARDHEIGHT ; j++ )
        {
            ...
            for( k = 0 ; ( ( i + k ) < BOARDWIDTH ) && ( ( j - k ) >= 0 ) ; k++ )
            {
                ...
                else
                {
                    ...
                    {
                        iMatchCount++;
                        // 연속된 블록이 세 개면 모아둔 이전 블록 세 개를 지울 블록으로 표시
                        if( iMatchCount == BLOCKCOUNT )
                        {
                            for( l = 0 ; l < iMatchCount ; l++ )
                            {
...
void CompactBlockOnBoard( void )
{
    ...
    for( i = 0 ; i < BOARDWIDTH ; i++ )
    {
        ...
        for( j = BOARDHEIGHT - 1 ; j >= 0 ; j-- )
        {
            ...
            // 중간이면 빈 블록이 검출되었고 현재 위치에 블록이 있으면 아래로 이동
...
void EraseAllContinuousBlockOnBoard( QWORD qwWindowID )
{
    ...
    // 게임 정보를 자료구조에 있는 삭제한 블록 빌드를 초기화
...
void DrawInformation( QWORD qwWindowID )
{
    ...
    // 게임 정보 영역의 배경을 출력
    DrawRect( qwWindowID, 1, WINDOW_TITLEBAR_HEIGHT - 1, WINDOW_WIDTH - 2, WINDOW_TITLEBAR_HEIGHT + INFORMATION_HEIGHT, RGB( 55, 215, 47 ), TRUE );
    ...
    DrawText( qwWindowID, ( WINDOW_WIDTH - iLength * FONT_ENGLISHWIDTH ) / 2, WINDOW_TITLEBAR_HEIGHT + 2, RGB( 255, 255, 255 ), RGB( 55, 215, 47 ), vcBuffer, strlen( vcBuffer ) );
}
