...
int HexToString( QWORD qwValue, char* pcBuffer )
{
    ...
    // 버퍼 1의 자리부터 16, 256, ...의 자리 순서로 숫자 삽입
    for( i = 0 ; qwValue > 0 ; i++ )
    {
...
BOOL GetOverlappedRectangle( const RECT* pstArea1, const RECT* pstArea2, RECT* pstIntersection )
{
    ...
    // 계산한 시작점의 위치가 끝점보다 크다면 두 사각형은 겹치지 않음
    if( iMinY2 < iMaxY1 )
    {
        return FALSE;
    }
...
//  윈도우 내부를 기준으로 한 X, Y좌표로 화면 좌표를 변환
...
void SetRectangleData( int iX1, int iY1, int iX2, int iY2, RECT* pstRect )
{
    ...
    // y1 < y2가 되도록 RECT 자료구조에 Y 좌표를 설정
    if( iY1 < iY2 )
    {
        pstRect->iY1 = iY1;
        pstRect->iY2 = iY2;
