...
int Main( char* pcArgument )
{
    ...
    // 파일을 디렉토리에서 찾은 뒤에 파일 크기만큼 메모리를 할당하여 파일을 저장
    ...
    while( 1 )
    {
        ...
        case EVENT_WINDOW_RESIZE:
            ...
            // 변경된 화면 크기로 다시 라인 수와 라인당 문자 수, 파일 오프셋을 계산 하고
...
BOOL ReadFileToBuffer( const char* pcFileName, TEXTINFO* pstInfo )
{
    ...
    // 디렉토리 핸들을 반환. 핸들을 반환하지 않으면 메모리가 해제되지 않고 남으므로 꼭 해제해야 한다.
...
BOOL DrawTextBuffer( QWORD qwWindowID, TEXTINFO* pstInfo )
{
    ...
    //------------------------------------------------------------------------------------------
    // 현재 라인에서 남은 라인 수와 흰 화면에 출력할 수 있는 라인 수를 비교하여 작은 것을 선택
