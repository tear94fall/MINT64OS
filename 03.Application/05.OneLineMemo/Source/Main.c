    ...
    while( 1 )
    {
        ...
        case EVENT_KEY_DOWN:
            ...
            case KEY_LALT:
                ...
                // 영문 입력 모드 중에서 Alt 키가 눌러지면 한글 조합용 버퍼를 초기화
                else
                {
                    stBufferManager.iInputBufferLength = 0;
            ...
            default:
                ...
                if( ( bHangulMode == TRUE ) && ( stBufferManager.iOutputBufferLength + 2 <= MAXOUTPUTLENGTH ) )
                {
                    ...
                    {
                        // 조합이 완료된 버퍼에 깂이 있는지 확인하여 윈도우 화면에
                        // 출력할 버퍼로 복사
                        if( stBufferManager.vcOutputBufferForComplete[ 0 ] != '\0' )
                        {
                            ...
                            // 공간이 없다면 키 입력 버퍼와 조합중인 한글 버퍼를 모두 초기화
                    ...
                    // 조합에 실패하면 입력 버퍼에 마지막으로 입력된 값이 한글 자 모음이 아니므로
            ...
            //------------------------------------------------------------------------------------------
            // 화면 출력 버퍼에 있는 문자열과 조합 중인 한글을 출력하고 커서의 위치를 갱신
            //------------------------------------------------------------------------------------------
