...
static void kTestTask3( void )
{
    ...
    // 현재 수행 중인 로컬 APIC ID를 저장했다가 태스크가 부하 분산되어 다른 코어로
    // 옮겨갔을 때 메시지를 출력
...
static void kShowTaskList( const char* pcParameterBuffer )
{
    ...
    for( i = 0 ; i < TASK_MAXCOUNT ; i++ )
    {
        ...
        {
            // 태스크가 6개 출려될 때마다 계속 태스크 정보를 표시할지 여부를 확인
...
static void kCPULoad( const char* pcParameterBuffer )
{
    ...
    for( i = 0 ; i < kGetProcessorCount() ; i++ )
    {
        ...
        kSPrintf( vcBuffer, "Core %d : %d%\n", i, kGetProcessorLoad( i ) );
        kPrintf( "%s", vcBuffer );
...
static void kPrintNumberTask( void )
{
    ...
    // 50ms 정도 대기하여 콘솔 쉘이 출력하는 메시지와 겹치지 않도록 함
...
//  하드 디스크에서 파라미터로 넘어온 LBA 어드레스로부터 섹터 수만큼 씀
static void kWriteSector( const char* pcParameterBuffer )
{
    ...
    s_dwWriteCount++;
    // 버퍼를 할당받아 데이터를 채움
    // 패턴은 4바이트의 LBA 어드레스와 4바이트 쓰기가 수행된 횟수로 생성
    pcBuffer = kAllocateMemory( iSectorCount * 512 );
    for( j = 0 ; j < iSectorCount ; j++ )
    {
        for( i = 0 ; i < 512 ; i += 8 )
...
static void kWriteDataToFile( const char* pcParameterBuffer )
{
    ...
    {
        bKey = kGetCh();
        // 엔터 키면 연속 3번 눌러졌는가 확인하여 루프를 빠져나감
        if( bKey == KEY_ENTER )
        {
            iEnterCount++;
            if( iEnterCount >= 3 )
...
static void kReadDataFromFile( const char* pcParameterBuffer )
{
    ...
    while( 1 )
    {
        ...
        // 만약 엔터 키면 엔터 키 횟수를 증가시키고 20라인까지 출력했다면 더 출력할지 여부를 물어봄
        if( bKey == KEY_ENTER )
        {
            iEnterCount++;
...
static void kTestFileIO( const char* pcParameterBuffer )
{
    ...
    // 열린 핸들로 읽기 수행 후 데이터 검증, Random Write로 데이터가 잘못
    // 저장될 수 있으므로 검증은 4MB 전체를 대상으로 함
    ...
    // 파일이 열려 있는 상태이므로 파일을 지우려고 하면 실패 해야함
...
static void kDownloadFile( const char* pcParameterBuffer )
{
    ...
    kClearSerialFIFO();
    //===================================================================================
    // 데이터 길이가 수신될 때까지 기다린다는 메시지를 출력하고, 4바이트를 수신한 뒤 ACK 전송
    //===================================================================================
    ...
    //=================================================================================
    // 전체 데이터의 크기와 실제로 수신받은 데이터의 크기를 비교하여 성공 여부를 출력한 뒤,
