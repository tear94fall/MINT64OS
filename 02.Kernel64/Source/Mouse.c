...
void kEnableMouseInterrupt( void )
{
    ...
    // 입력 버퍼(포트 0x60)에 데이터가 비어 있으면 출력 포트에 값을 쓰는 커맨드와 커맨드 바이트 전송
    for( i = 0 ; i < 0xFFFF ; i++ )
    {
...
BOOL kAccumulateMouseDataAndPutQueue( BYTE bMouseData )
{
    ...
    case 2:
        ...
        // 그외의 경우는 수신된 바이트 수 초기화
    default:
        gs_stMouseManager.iByteCount = 0;
        break;
    }

    // 3바이트가 모두 수신되면 마우스 큐에 삽입하고 수신된 횟수를 초기화
...
BOOL kGetMouseDataFromMouseQueue( BYTE* pbButtonStatus, int* piRelativeX, int* piRelativeY )
{
    ...
    *pbButtonStatus = stData.bButtonStatusAndFlag & 0x7;
    ...
    // X, Y의 이동 거리 설정
    // X의 부호 비트는 비트 4에 있으며 1로 설정되면 음수임
    *piRelativeX = stData.bXMovement & 0xFF;
    if( stData.bButtonStatusAndFlag & 0x10 )
    {
        // 음수이므로 아래 8비트에 X 이동 거리를 설정한 후 상위 비트를 모두 1로 만들어 부호 비트를 확장
        *piRelativeX |= ( 0xFFFFFF00 );
    }

    // Y의 부호 비트는 비트 5에 있으면 1로 설정되면 음수임
    // 아래 방향으로 갈수록 Y값이 증가하는 화면 좌표와 달리 마우스는 위쪽 방향으로 갈수록
    // 값이 증가하므로 계산이 끝난 후 부호를 뒤짚음
    *piRelativeY = stData.bYMovement & 0xFF;
    if( stData.bButtonStatusAndFlag & 0x20 )
    {
        // 음수이므로 아래 8비트에 Y 이동 거리를 설정한 후 상위 비트를 모두 1로 마들어 부호 비트를 확장
        *piRelativeY |= ( 0xFFFFFF00 );
    }

    // 마우스의 Y축 증감 방향은 화면 좌표와 반대이므로 Y 이동 거리에 -하여 방향을 바꿈
...
{
    // 출력 버퍼(포트 0x60)를 읽기 전에 상태 레지스터(포트 0x64)를 읽어서
