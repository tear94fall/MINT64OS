...
static int kAllocationBuddyBlock( QWORD qwAlignedSize )
{
    ...
    {
        // 블록 리스트와 비트맵을 확인하여 블록이 존재하는지 확인
...
BOOL kFreeMemory( void* pvAddress )
{
    ...
    // 넘겨 받은 어드레스를 블록 풀을 기준으로 하는 어드레스로 변환하여 할당 했던 블록의 크기를 검색
    ...
    // 버디 블록의 최소 크기를 블록 리스트 인덱스로 쉬프트 하여 할당된 블록의 크기 계산
