...
QWORD kExecuteProgram( const char* pcFileName, const char* pcArgumentString, BYTE bAffinity )
{
    ...
    // 디렉토리 핸들을 반환, 핸들을 반환하지 않으면 메모리가 해제되지 않고 남으므로 꼭 해제해야 함
    ...
    //-------------------------------------------------------------------------------------------
    // 파일 전체를 저장할 수 있는 임시 버퍼를 할당받아서 파일의 내용을 모두 저장
    //--------------------------------------------------------------------------------------------
    // 메모리 할당
...
static BOOL kLoadProgramAndRelocation( BYTE* pbFileBuffer, QWORD* pqwApplicationMemoryAddress, QWORD* pqwApplicationMemorySize, QWORD* pqwEntryPointAddress )
{
    ...
    {
        // 가장 마지막 섹션인지 확인. 이 값으로 프로그래밍이 사용할 전체 메모리 크기를 알 수 있음
...
static BOOL kRelocation( BYTE* pbFileBuffer )
{
    ...
    for( i = 1 ; i < pstELFHeader->e_shnum ; i++ )
    {
        ...
        for( j = 0 ; j < pstSectionHeader[ i ].sh_size ; )
        {
            ...
            // 공통 타입 심볼은 지원하지 않으므로 오류를 표시하고 종료
            ...
            case R_X86_64_PC64:
                ...
                // B(sh_addr) + A(r_addend)로 계산하는 타입
...
QWORD kCreateThread( QWORD qwEntryPoint, QWORD qwArgument, BYTE bAffinity, QWORD qwExitFunction )
{
    ...
    *( ( QWORD* ) pstTask->stContext.vqRegister[ TASK_RSPOFFSET ] ) = qwExitFunction;
    ...
    // 첫 번째 파리미터로 사용되는 RDI 레지스터에 인자를 삽입
