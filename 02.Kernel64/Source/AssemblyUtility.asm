...
kInPortWord:
    ...
    mov rax, 0          ; RAX 레지스터를 초기화
    in ax, dx           ; DX 레지스터에 저장된 포트 어드레스에서 두 바이트를 읽어 AX 레지스터에 저장,
                        ; AX 레지스터는 함수의 반환 값으로 사용됨
...
kReadRFLAGS:
    ...
    pop rax             ; 스택에 저정된 RFLAGS 레지스터를 RAX 레지스터에 저장하여
                        ; 함수의 반환 값으로 설정
    ret
...
; 콘텍스트를 복원하는 매크로
%macro KLOADCONTEXT 0       ; 파라미터를 전달 받지 않는 KSAVECONTEXT 매크로 정의
...
kSwitchContext:
    ...
    ; 저장한 레지스터를 복구한 후 인터럽트가 발생했 때처럼 나머지 콘텍스트를 모두 저장
    pop rax
    pop rbp
