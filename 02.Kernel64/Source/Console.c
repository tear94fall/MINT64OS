...
// 그래픽 모드로 시작했을때 GUI 콘솔 쉘 윈도우로 전달된 키 이벤트를 콘솔 쉘 태스크로 전달하는 큐 버퍼
...
void kInitializeConsole( int iX, int iY )
{
    ...
    {
        // 그래픽 모드로 시작헀으면 그래픽 모드용 화면 버퍼로 설정
...
void kSetCursor( int iX, int iY )
{
    ...
    // 그래픽 모드로 시작헀으면 화면 버퍼에 출력한 커서의 위치를 옮겨줌
    else
    {
        // 이전에 커서가 있던 위치가 그대로 커서로 남아 있으면 커서를 지움
...
BYTE kGetCh( void )
{
    ...
    while( 1 )
    {
        ...
        // 그래픽 모드면 그래픽 모드용 키 큐에서 값을 가져옴
        else
        {
            while( kGetKeyFromGUIKeyQueue( &stData ) == FALSE )
            {
                // 그래픽 모드에서 동작하는 중에 쉘 태스크를 종료해야 되면 루프를 종료
